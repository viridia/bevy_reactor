...
// Converts the 2D drawing coordinates into 3D coordinates by projecting onto an axis-aligned
// plane in 3-space.
...
impl DrawPlane {
    pub fn project(&self, x: f32, y: f32, z: f32) -> Vec3 {
        match self {
            DrawPlane::XY => Vec3::new(x, y, z),
            DrawPlane::XZ => Vec3::new(x, z, y),
            DrawPlane::YX => Vec3::new(y, x, z),
            DrawPlane::YZ => Vec3::new(y, z, x),
            DrawPlane::ZX => Vec3::new(z, x, y),
            DrawPlane::ZY => Vec3::new(z, y, x),
        }
    }
}
...
impl ShapeBuilder {
    ...
    /// Set which plane we're drawing on.
    #[inline]
    pub fn with_draw_plane(&mut self, draw_plane: DrawPlane) -> &mut Self {
        self.draw_plane = draw_plane;
        self
    }

    /// Sets the offset from the draw plane
    #[inline]
    pub fn with_offset(&mut self, offset: f32) -> &mut Self {
        self.offset = offset;
        self
    }
    ...
    pub fn push_vertex(&mut self, x: f32, y: f32, z: f32) -> &mut Self {
        self.vertices
            .push(self.draw_plane.project(x, y, z + self.offset));
        self
    }

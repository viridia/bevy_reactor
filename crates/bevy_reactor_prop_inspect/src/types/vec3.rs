...
pub fn vec3_field(field: Arc<Inspectable>) -> impl SceneList {
    ...
    let field_copy4 = field_copy.clone();
    let field_copy5 = field_copy.clone();
    let field_copy6 = field_copy.clone();
    bsn_list!{
        @field_group()
        Children [
            @field_label(field)
